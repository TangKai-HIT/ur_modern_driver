...
bool RTPublisher::publishPoses(RTShared& packet, Time& t)
{
  //Transform transform;
  auto tv = packet.tool_vector_actual;
  geometry_msgs::PoseStamped tool_pose_msg;
  tool_pose_msg.header.stamp = t;
  tool_pose_msg.pose.position.x =tv.position.x;
  tool_pose_msg.pose.position.y =tv.position.y;
  tool_pose_msg.pose.position.z =tv.position.z;
  tool_pose_pub_.publish(tool_pose_msg);
  return true;
 }
...
bool RTPublisher::publish(RTShared& packet)
{
  ...
  if (!temp_only_)
  {
    //res = publishJoints(packet, time) &&publishPoses(packet,time)&& publishWrench(packet, time) && publishTool(packet, time) &&
